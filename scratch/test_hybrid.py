...
def find_obscure_code(shoppable_codes):
    ...
    cursor.execute("SELECT code, code_type, description FROM procedures WHERE code_type IN ('CPT', 'HCPCS') LIMIT 500;")
    rows = cursor.fetchall()
    conn.close()
    
    for code, code_type, desc in rows:
        code_clean = code.strip().upper()
        if code_clean and code_clean not in shoppable_codes:
            print(f"Selected obscure code: '{code}' ({code_type}) - Description: {desc}")
            return code, desc
            
    # Default fallback
    return "99213", "Office visit"
...
def test_search(query):
    ...
    if context:
        ...
        print(f"Results list size: {len(context.get('grouped_results'))}")
    else:
...
def main():
    ...
    obscure_code, obscure_desc = find_obscure_code(shoppable)
    
    print("\n--- TEST 1: Common / Shoppable Search ('mri') ---")
    # This should hit in_shoppable.sqlite3 and finish instantly
    test_search("mri")
    
    print("\n--- TEST 2: Common / Shoppable Code Search ('70551') ---")
    # 70551 is Brain MRI, which is in shoppable list
    test_search("70551")
    
    print(f"\n--- TEST 3: Obscure / Fallback Search ('{obscure_code}') ---")
    # This is not in shoppable database; it should trigger fallback and query in_full.sqlite3 successfully
    test_search(obscure_code)
